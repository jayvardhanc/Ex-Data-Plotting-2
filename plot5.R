...
g<-ggplot(data,aes(factor(year),Emissions,width=0.5))+geom_bar(stat="identity",fill="cyan")+xlab("year")+ylab(expression("Total "*PM[2.5]*" emissions (in tons)"))+ggtitle(expression(PM[2.5]*" emissions from vehicle sources in Baltimore City (1999-2008)"))
print(g)
dev.off()